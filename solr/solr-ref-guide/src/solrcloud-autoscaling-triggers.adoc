...
`metric`::
(string, required) The metric property name to be watched in the format metric:__group__:__prefix__, e.g., `metrics:solr.node:CONTAINER.fs.coreRoot.usableSpace`.
...
----
{
  "set-trigger": {
    ...
    "event": "metric",
    "waitFor": "5s",
    "metric": "metrics:solr.node:CONTAINER.fs.coreRoot.usableSpace",
    "below": 107374182400,
